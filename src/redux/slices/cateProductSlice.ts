...
import { createCateProduct, getAllCateProduct, readCateProduct, removeCateProduct, updateCateProduct } from "../../api-cilent/CatePro"
...
export const addCatePro = createAsyncThunk("catepro/create",async(cateproduct:ICatePro)=>{
        const res = await createCateProduct(cateproduct);
        return res
    }
)
export const updateCatePro = createAsyncThunk("catepro/edit", async ( datacate : any) => {
    const res = await updateCateProduct(datacate);
    return res;
})
export const deleteCatePro = createAsyncThunk("category/delete", async (id : any) => {
...
const cateProSlice = createSlice({
    ...
    extraReducers:(build) =>{
        ...
        build.addCase(updateCatePro.fulfilled,(state,{payload})=>{
            state.cateproduct = state.cateproducts.map((item) => item._id === payload._id)
        })
