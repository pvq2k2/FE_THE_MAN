...
import { createCatePost, getAll, read, remove, update } from "../../api-cilent/CatePost";
...
export const getAllCatePosts = createAsyncThunk(
    "catepost/getall",
    async()=>{
...
const catePostSlice = createSlice({
    ...
    extraReducers:(build)=>{
        ...
        build.addCase(deleteCatePost.fulfilled,(state,{payload})=>{
            state.cateposts = state.cateposts.filter(
        ...
        build.addCase(updateCatePost.fulfilled,(state,{payload})=>{
            ...
            item._id === payload?._id ? payload:item)as ICatePost[];
